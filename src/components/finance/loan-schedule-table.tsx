...
import { hasLevelPayment, loanSchedule, repaymentsMade, type LoanTerms } from "@/lib/finance";
 ...
 *  a rate change or a prepayment gets a line of its own where it comes. Where there is no
...
export function LoanScheduleTable({ terms, currency, today, className }: {
  ...
  const level = hasLevelPayment(terms.method);
  return (
      ...
      <div ref={scroller} className="@container max-h-96 overflow-auto rounded-lg ring-1 ring-foreground/10">
        <table className="w-full text-xs tabular-nums [&_td]:px-1 [&_th]:px-1 [&_tr>*:first-child]:pl-1.5 [&_tr>*:last-child]:pr-1.5 @md:[&_td]:px-2 @md:[&_th]:px-2">
          ...
          <tbody className="[&_td]:py-1.5">
            {periods.map((p, i) => {
              const rateChanged = i > 0 && p.rate !== periods[i - 1].rate;
              const paymentChanged = level && i > 0 && p.payment !== periods[i - 1].payment && p.balance > 0;
              const cleared = p.balance === 0 && p.principal === 0;
              return (
                <Fragment key={p.n}>
                  {p.prepaid?.map((x) => (
                    <tr key={x.paid_on} className="border-t border-border/60">
                      <td colSpan={6} className="text-muted-foreground">
                        Prepaid {amount(x.amount)} on {dayLabel(x.paid_on)}
                        {cleared ? ", repaying the loan" : !rateChanged && paymentChanged && `, ${amount(p.payment)} a month from then`}
                      </td>
                    </tr>
                  ))}
                  {rateChanged && (
                    <tr className="border-t border-border/60">
                      <td colSpan={6} className="text-muted-foreground">
                        From {dayLabel(p.date)}: {p.rate}% a year{paymentChanged && `, ${amount(p.payment)} a month`}
                      </td>
                    </tr>
                  )}
                  <tr
